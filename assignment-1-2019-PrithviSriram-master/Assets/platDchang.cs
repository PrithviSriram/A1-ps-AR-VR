...
public class platDchang : MonoBehaviour
{
    public Camera cam;
    ...
    IEnumerator sizedown_goup(RaycastHit hit)
    {
        if (hit.collider.name == "PointDPlatform" | hit.collider.name == "PointDWall1" | hit.collider.name == "PointDWall2" | hit.collider.name == "PointDWall3" | hit.collider.name == "PointDWall4")
        {
            ...
            while (true)
            {
                ...
                cursize = transform.localScale;
                //Debug.Log(cursize);
                if (transform.localScale.z >= 4.9)//if (platd.transform.localScale.z <= 1.2)
                {
                    Debug.Log("b");
                    break;
            ...
            while (true)
            {
                ...
                if (transform.localScale.z <= 2.3)//if (platd.transform.localScale.z <= 1.2)
                {
                    Debug.Log("c");
                    break;
                }
